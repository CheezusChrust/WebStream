...
WebStream.StreamsWaitingForDownload = WebStream.StreamsWaitingForDownload or {}
WebStream.DownloadsReady = WebStream.DownloadsReady or {}
WebStream.Server = ""
...
local function TransmitData(Stream)
    ...
    http.Post(WebStream.Server, {
        ["data"] = chunkData,
        ["name"] = chunkName
    }, function(body, _, _, responseCode)
        ...
        if Stream.retries <= GetConVar("webstream_maxretries"):GetInt() then
            TransmitData(Stream)
        else
            if Stream.onFailure then Stream.onFailure() end
...
local function ReceiveData(Stream)
    ...
    http.Post(WebStream.Server, {
        ["name"] = Stream.chunkNames[Stream.progress]
    }, function(body, _, _, responseCode)
        if not Stream then return end
        if responseCode ~= 200 then
            ...
            if Stream.progress == #Stream.chunkNames then
                ...
                Stream.callback(util.Base64Decode(data))
